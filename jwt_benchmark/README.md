...
The main summary file contains metrics such as:

- average token generation time,
- median token generation time,
- token generation standard deviation,
- average token verification time,
- median token verification time,
- token verification standard deviation,
- average authenticated request header size,
- average token response body size,
...
- `charts/token_gen_time.png`,
- `charts/token_ver_time.png`,
- `charts/sizes.png`,
- `charts/response_time.png`,
- `charts/token_sizes.png`,
- `charts/latency_grouped.png`,
- `charts/combined_benchmark.png`.

## How to change benchmark settings

Edit `benchmark/config.py` to change:

- number of warm-up runs: `NUM_WARMUP`,
- number of measured runs: `NUM_MEASURED`,
- JWT payload fields: `JWT_PAYLOAD`,
- algorithms included in the benchmark: `ALGORITHMS`,
- nested hybrid configurations: `NESTED_CONFIGS`,
- HTTP test scenarios: `TEST_SCENARIOS`.

## Troubleshooting

If `python3 -c "import oqs"` fails, the PQC algorithms cannot run until `liboqs-python` is installed correctly.

If some algorithms are missing, verify that your liboqs build enables the required signature mechanisms:

```bash
python3 -c "import oqs; print(oqs.get_enabled_sig_mechanisms())"
```

If the charts do not update, delete old files in `charts/` and rerun:
